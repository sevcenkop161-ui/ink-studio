...
import { works, workCategories, type WorkCategory, type WorkSize } from "@/data/works";
...
export function Works() {
  ...
  const selectedWork = works.find((work) => work.slug === selectedSlug) ?? null;
  const selectedArtist = selectedWork
    ? artists.find((artist) => artist.slug === selectedWork.artistSlug)
    : undefined;
          ...
          <div>
            ...
            <div className="space-y-3 p-6">
              ...
              <p className="text-text-secondary">
                {t(`items.${selectedWork.slug}.description`)}
              </p>
              {selectedArtist && (
                <Link
                ...
                >
                  {t("byArtist", { name: selectedArtist.name })} ·{" "}
                  {t("viewProfile")} →
                </Link>
              )}
            </div>
