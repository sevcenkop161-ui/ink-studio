...
type Props = {
  ...
  errorReason: "conflict" | "generic" | null;
};
...
export function ConfirmationStep({ data, artists, services, errorReason }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <dl className="divide-y divide-border rounded-lg border border-border">
        <Row label={t("summary.service")} value={service?.name ?? "—"} />
      ...
      {errorReason === "conflict" && (
        <p className="text-sm text-red-400">{t("errors.conflict")}</p>
      )}
      {errorReason === "generic" && (
